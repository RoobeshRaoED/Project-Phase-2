...
function sidebarItemsFor(role){
  if(role==='admin'){
    return [
      ...
      {view:'usermgmt', ic:'👤', label:'Customer Management'},
...
const VIEWS_ADMIN = ['admindash','usermgmt','adminmgmt','employeemgmt','legalmgmt','bankofficemgmt','biddermgmt','profile'];
...
function go(view){
  ...
  $all('.nav-item').forEach(b=>b.classList.toggle('active',
    ...
    (['biddermgmt','adminmgmt'].includes(view) && b.dataset.view==='usermgmt')
  ));
...
function render(view){
  ...
  switch(view){
    ...
    case 'usermgmt': main.innerHTML = viewCustomerManagement('customer'); break;
...
function viewEligibility(){
  ...
  <div class="grid grid-2">
    <div class="card">
      ...
      <div class="field"><label>Address<span class="req">*</span></label><textarea id="elAddress" rows="2" placeholder="Enter your current address">${escapeHtml(emp.address||'')}</textarea><div class="error hidden" id="err_elAddress"></div></div>
      <div class="field">
        <label>Address Proof (for verification)</label>
        <div class="upload-box" onclick="document.getElementById('elAddressProof').click()">
          <input type="file" id="elAddressProof" accept=".pdf,.jpg,.jpeg,.png" onchange="handleEligAddressProof(this)">
          📎 ${elAddressProofName ? 'Replace file — '+elAddressProofName : 'Click to upload address proof document'}
        </div>
      </div>
...
let elAddressProofName = '';
function handleEligAddressProof(input){
  if(input.files && input.files[0]){
    elAddressProofName = input.files[0].name;
    toast('File Selected', elAddressProofName+' will be used for address verification.', 'success');
    const box = input.closest('.upload-box');
    if(box) box.innerHTML = `<input type="file" id="elAddressProof" accept=".pdf,.jpg,.jpeg,.png" onchange="handleEligAddressProof(this)">📎 Replace file — ${escapeHtml(elAddressProofName)}`;
  }
}
...
function runEligibility(silent){
  ...
  if(!address) return fieldError($('#elAddress'), $('#err_elAddress'), 'Address is required.');
  fieldOk($('#elAddress'), $('#err_elAddress'));
  ...
  const u = currentUser();
  u.employment = {...u.employment, monthlyIncome:income, otherEMIs:otherEmi, pan, address, type:empType, desiredAmount:amount};
...
function freshApplyData(){
  ...
  return {
    ...
    personal:{ name:u.name, mobile:u.mobile, email:u.email, address:{doorNo:'',street:'',city:'',state:'',pincode:''} },
...
function viewCustomerManagement(activeTab){
  ...
  const tabs = [
    {key:'customer', label:'Customers', view:'usermgmt'},
  ...
  return `
  <div class="page-head"><h1>Customer Management</h1><p>Manage customer, bidder and administrator accounts from one place.</p></div>
  ...
  <div class="card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
      <h3 style="margin:0;">${activeTab==='customer'?'👥':activeTab==='bidder'?'🔨':'🛡️'} ${label}s (${list.length})</h3>
      ${activeTab!=='customer'? `<button class="btn btn-primary btn-sm" onclick="openAddStaffModal('${activeTab}')">+ Add ${label}</button>` : ''}
    </div>
...
const SEARCH_INDEX = [
  ...
  {label:'Customer Management', view:'usermgmt'}, {label:'Employee Management', view:'employeemgmt'},
