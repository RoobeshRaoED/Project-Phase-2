...
function stepProperty(){
  ...
  <div class="field">
    <label>Upload Property Address Proof (optional)</label>
    ...
    <div class="timeline">
      ${stages.map((s,i)=>`
        <div class="tl-item ${s.status==='completed'?'done':s.status==='current'?'current':''}">
          ...
          <div class="tl-body">
            <h5>${s.name}</h5>
            <p>${s.status==='completed'?'Completed':s.status==='current'?'In Progress':'Upcoming'} · Expected: ${fmtDate(s.expectedDate)} · ${s.officer}</p>
          </div>
        </div>`).join('')}
    </div>
  </div>
