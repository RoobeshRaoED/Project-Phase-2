...
const VIEWS_ADMIN = ['admindash','usermgmt','employeemgmt','legalmgmt','bankofficemgmt','biddermgmt','profile'];
...
function render(view){
  ...
  switch(view){
    ...
    case 'biddermgmt': main.innerHTML = viewStaffManagement('bidder'); break;
...
function stepLoanDetails(){
  ...
  <div class="field"><label>Loan Amount (₹)</label><input type="number" id="apAmount" min="200000" max="200000000"  value="${d.loanAmount}"><div class="error hidden" id="err_apAmount"></div></div>
...
/* ================================================================
   PUBLIC HOMEPAGE — NAV, STAKEHOLDER QUICK LOGIN, POLICY, 404 ROUTING
   ================================================================ */
const DEMO_CREDS = {
  admin:      {id:'admin@hlms.example.com',      pass:'Admin@123',  label:'Admin'},
  legal:      {id:'legal@hlms.example.com',      pass:'Legal@123',  label:'Legal Team'},
  bankoffice: {id:'bankoffice@hlms.example.com', pass:'Bank@123',   label:'Bank Office'},
  bidder:     {id:'bidder@hlms.example.com',     pass:'Bidder@123', label:'Bidder'}
};
const PUBLIC_ANCHORS = ['home','about','products','stakeholders','howitworks','policy','contact',''];
...
function scrollToSection(id){
  showHome();
  document.getElementById('homeNavLinks') && document.getElementById('homeNavLinks').classList.remove('open');
  setTimeout(()=>{
    const el = document.getElementById(id);
    if(el) el.scrollIntoView({behavior:'smooth', block:'start'});
  },30);
}
/* Quick login for a demo stakeholder role (Admin / Legal / Bank Office / Bidder). */
function quickLogin(role){
  ...
  setTimeout(()=>{
    ...
    if(c){ idEl.value = c.id; passEl.value = c.pass; }
    idEl.focus();
  },0);
}
function goRegisterAs(role){
  ...
  setTimeout(()=>{
    const sel = $('#regRole');
    if(sel && role){ sel.value = role; toggleRegRoleHint(); }
  },0);
}
function show404(){
  ...
  $('#notFoundScreen') && $('#notFoundScreen').classList.remove('hidden');
  window.scrollTo(0,0);
}
/* Handles top-level (logged-out) navigation: home anchors, #login, #register, or a 404. */
function publicRoute(){
  if(currentUser()) return; // the in-app router (go/hashchange) owns navigation once logged in
  const hash = location.hash.replace('#','');
  if(hash==='login'){ openAuth('login'); return; }
  if(hash==='register'){ openAuth('register'); return; }
  if(PUBLIC_ANCHORS.includes(hash)){
    showHome();
    if(hash) setTimeout(()=>{ const el=document.getElementById(hash); if(el) el.scrollIntoView({block:'start'}); },30);
    return;
  }
  show404();
}
